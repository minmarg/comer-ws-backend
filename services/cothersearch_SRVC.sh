...
## Initiate a profile-profile alignment search by submitting a job to a 
## workload manager's queue
...
usage="
Initiate a profile-profile alignment search by submitting a job to a workload 
manager's queue.
(C)2020 Mindaugas Margelevicius, Institute of Biotechnology, Vilnius University
...
comerlogfile="${filename}__cother_out.log"
reslstfile="${filename}__cother_out.lst"
resultsfile="${filename}__cother_out.tar"
errorfile="${filename}.err"
#outfile=${filename}.out

sendemail="${dirname}/../bin/sendemail.pl"
cotherpl="${dirname}/../bin/cother_wrapper.pl"
...
if [[ ! -f "${cotherpl}" ]]; then fail+="ERROR: $ID: Program not found: ${cotherpl}\n"; fi
...
if [ -z "$fail" ]; then
  ...
  ##NOTE: Always set 1 for the number of jobs per GPU as distance predictions may require all GPU resources
  njobspergpu=1
  #
  if [ -n "${ngpusyst}" ]; then
    ngpusyst_f=${ngpusyst}
...
##
$SUBPROG ${SLURM_OPTS} -n ${ncpus} -t 1440 -J comer-ws-cother -o "${shoutfile}" \
...
if [[ -n "${ngpunits}" && ${ngpunits} -gt 0 ]]; then
  ...
  ## NOTE: this os not a general solution since jobs may terminate, making a 
...
if [ -z "\${bfail}" ]; then
  cmd="$cotherpl --in \"${inpfile}\" --opt \"${optfile}\" --status \"${statusfile}\""
  cmd+=" --comerlog \"${comerlogfile}\" --reslst \"${reslstfile}\""
