...
comerlogfile="${filename}__comer_out.log"
reslstfile="${filename}__comer_out.lst"
resultsfile="${filename}__comer_out.tar"
errorfile="${filename}.err"
#outfile=${filename}.out

sendemail="${dirname}/../bin/sendemail.pl"
comerpl="${dirname}/../bin/comer_wrapper.pl"
...
if [[ ! -f "${comerpl}" ]]; then fail+="ERROR: $ID: Program not found: ${comerpl}\n"; fi
...
##
$SUBPROG ${SLURM_OPTS} -n ${ncpus} -t 1440 -J comer-ws-search -o "${shoutfile}" \
...
if [ -z "\${bfail}" ]; then
  cmd="$comerpl --in \"${inpfile}\" --opt \"${optfile}\" --status \"${statusfile}\""
